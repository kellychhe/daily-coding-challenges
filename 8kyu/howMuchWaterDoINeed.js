...
function howMuchWater(water, load, clothes){
    let waterUsed = 0
    let maxLoad = 2 * load
    if (clothes <= maxLoad && clothes >= load){
      waterUsed = water * 1.1 ** (clothes-load)
      return parseFloat(waterUsed.toFixed(2))
  ...
  //if else clothes is  less than two times the load then run the cycle, if not return a string
  //if is returning how much water is used for a given load